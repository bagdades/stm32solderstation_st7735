...
static char* state_tempstr[2] = { "250^C", "400^C" };
...
static uint8_t Cal_draw(screen_t *scr){
	if(error==1){
		...
		ucg_WriteString(&ucg, 0, 10, strings[lang].CAL_Error);
		ucg_WriteString(&ucg, 0, 25, strings[lang].CAL_Aborting);
	}
...
static uint8_t Cal_Start_draw(screen_t *scr){
	...
	if(update_draw){
		...
		if(current_state<cal_finished){
			...
			if(current_state<cal_input_250){
				ucg_WriteString(&ucg, 0, 24, strings[lang].CAL_Wait);               // Draw current temp
				sprintf(str, "%3u^C", last_TIP_C);
				width = ucg_GetStrWidth(&ucg, default_font, str);
				ucg_WriteString(&ucg, ucg_GetXDim(&ucg) - width - 1, 24, str);
			}
			else{
				ucg_WriteString(&ucg, 0, 24, strings[lang].CAL_Measured);
				s-=10;
			}
			width = ucg_GetStrWidth(&ucg, default_font, state_tempstr[s]);
			ucg_WriteString(&ucg, ucg_GetXDim(&ucg) - width - 1, 6, state_tempstr[s]);
		}
		else if(current_state==cal_finished){
			for(uint8_t x=cal_250;x<(cal_400+1);x++){
				sprintf(str, "%s: %u", state_tempstr[x], adcCal[x]);
				ucg_WriteString(&ucg, 0, (x*14), str);
			}
			ucg_WriteString(&ucg, 0, 30, strings[lang].CAL_Success);
		}
		else if(current_state==cal_failed){
			ucg_WriteString(&ucg, 0, 24, strings[lang].CAL_Failed);
		}
...
static void Cal_Start_create(screen_t *scr) {
	...
	((button_widget_t*)w->content)->font=default_font;
	w->enabled=0;
