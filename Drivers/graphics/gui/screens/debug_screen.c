...
static uint8_t debug_Draw(screen_t *scr){
  if(scr->refresh==screen_Erased){
    ...
    ucg_WriteString(&ucg, 0, 0, "P");
    ucg_WriteString(&ucg, 0, 11, "I");
    ucg_WriteString(&ucg, 0, 22, "D");
    ucg_WriteString(&ucg, 64, 0, "AVG");
    ucg_WriteString(&ucg, 64, 11, "RAW");
    ucg_WriteString(&ucg, 64, 22, "SET");
    ucg_WriteString(&ucg, 64, 33, "ERR");
    ucg_WriteString(&ucg, 64, 44, "PWM");
    ucg_WriteString(&ucg, 64, 55, "PWR");
  }
...
static uint8_t pid_debug_Draw(screen_t * scr){
  if(update_draw || scr->refresh==screen_Erased){
    ...
    for(uint8_t x=1; x<(PID_SZ-1); x++){
	  ...
	  ucg_SetForeColor(&ucg, C_CYAN);
      ucg_DrawLine(&ucg, x+(ucg_GetXDim(&ucg)-PID_SZ), 20-pidPlot->p[prev], x+(ucg_GetXDim(&ucg)-PID_SZ+1), 20-pidPlot->p[pos]);
      ucg_DrawLine(&ucg, x+(ucg_GetXDim(&ucg)-PID_SZ), 42-pidPlot->i[prev], x+(ucg_GetXDim(&ucg)-PID_SZ+1), 42-pidPlot->i[pos]);
      ucg_DrawLine(&ucg, x+(ucg_GetXDim(&ucg)-PID_SZ), 63-pidPlot->d[prev], x+(ucg_GetXDim(&ucg)-PID_SZ+1), 63-pidPlot->d[pos]);
    }
  }
  return (default_screenDraw(scr));
}
