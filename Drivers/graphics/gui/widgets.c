...
tFont *default_font = (tFont*)&font_18m;

uint16_t fcolor;
uint16_t bcolor;
uint16_t default_color = C_CYAN;
...
void newWidget(widget_t **new, widgetType type, struct screen_t *scr){
	...
	switch(type){
		...
		case widget_bmp_button:
			...
			break;
		case widget_bmp:
			w->content = _malloc(sizeof(bmp_widget_t));
			break;
		default:
			Error_Handler();
	}
...
void widgetClearField(widget_t* w){
	...
	button_widget_t* button;
	bmp_widget_t* bmp;
	uint8_t cHeight = ucg_GetFont(&ucg)->chars->image->height;
	/* uint8_t r; */
	...
	if(w->frameType!=frame_combo){
		...
		if(w->parent->refresh < screen_Erase ){
			switch((uint8_t)w->type){
				case widget_bmp:
					bmp = (bmp_widget_t*)w->content;
					ucg_FillRectangle(&ucg, w->posX, w->posY, bmp->xbm->width, bmp->xbm->height);
					break;

				case widget_bmp_button:
					button = (button_widget_t*)w->content;
					ucg_FillRectangle(&ucg, w->posX, w->posY, button->xbm[0]+4, button->xbm[1]+4);
					break;

				case widget_button:
					button = (button_widget_t*)w->content;
					ucg_FillRectangle(&ucg, w->posX, w->posY, w->width, cHeight+1);
					break;

				case widget_display:
					ucg_FillRectangle(&ucg, w->posX, w->posY, w->width, cHeight+1);       // Draw black square to erase previous data
					break;
		...
		if( (sel && (sel->state==widget_edit) && (dis->type!=field_string) && (w->frameType==frame_auto)) || (w->frameType==frame_solid) ){
			...
			/* } */
			if(sel){
				ucg_FillRectangle(&ucg, w->posX, w->posY, w->width, cHeight + 2);
			}
			else{
				ucg_FillRectangle(&ucg, w->posX ,w->posY, w->width + 2, cHeight + 2);
			}
...
void widgetAlign(widget_t* w){
	...
	switch(w->type){
		case widget_button:
			button = (button_widget_t *)w->content;
			strWidth = ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), button->displayString);
			break;
		...
		case widget_multi_option:
			strWidth = ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), (char*)edit->options[*(uint8_t*)dis->getData()]);
			break;
		default:
			return;
	}
	...
	if(sel && (w->frameType!=frame_disabled)){      // If selectable, extra space for not overlapping the frame
		if(w->width < strWidth+7){                    // If width too small
			if(strWidth+7<ucg_GetXDim(&ucg)){                   // If fits oled size
	...
	switch(dispAlign){
		case align_disabled:
			break;
		case align_center:
			w->posX =(ucg_GetXDim(&ucg) - w->width)/2;
			break;
		case align_right:
			w->posX = ucg_GetXDim(&ucg) - w->width;
			break;
		case align_left:
		default:
			w->posX = 0;
	...
	switch(textAlign){
		...
		case align_left:
		default:
			if(sel && ((w->posX+3)<=ucg_GetXDim(&ucg))){
...
uint8_t default_widgetDraw(widget_t *w) {
	...
	if( dis ){
		if(ucg_GetFont(&ucg) != dis->font){
			ucg_SetFont(&ucg, dis->font);
		}
		cHeight = ucg_GetFont(&ucg)->chars->image->height;
	}
	else if((w->type == widget_button)||(w->type == widget_bmp_button)){
		button = (button_widget_t*)w->content;
		if(w->type == widget_button){
			if(ucg_GetFont(&ucg) != button->font){
				ucg_SetFont(&ucg, button->font);
			}
			cHeight = ucg_GetFont(&ucg)->chars->image->height;
		}
	}
	...
	if(refresh){
		...
		switch(w->type){
			...
			case widget_button:
				ucg_WriteString(&ucg, button->stringStart, w->posY + 2, button->displayString);
				break;
			...
			case widget_editable:
				if(dis->type == field_string){
					ucg_WriteString(&ucg,  dis->stringStart, w->posY+2,  dis->getData());
					if(sel->state == widget_edit){
						char t[20];
						strcpy(t,dis->getData());

						t[edit->current_edit+1]=0;
						uint8_t x2=ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), t);

						t[edit->current_edit]=0;
						uint8_t x1=ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), t);

						ucg_SetForeColor(&ucg, C_BLACK);
						ucg_FillRectangle(&ucg, dis->stringStart, w->posY+ cHeight, w->width, 2);

						ucg_SetForeColor(&ucg, w->fcolor);
						ucg_FillRectangle(&ucg,  dis->stringStart+x1, w->posY+ cHeight, x2-x1, 2);
					...
					ucg_WriteString(&ucg,w->posX, w->posY,  dis->displayString);
					/* ucg_WriteString(&ucg,dis->stringStart, w->posY+2,  dis->displayString); */
				}
				break;

			case widget_multi_option:
				ucg_WriteString(&ucg, dis->stringStart, w->posY+2, edit->options[*(uint8_t*)dis->getData()]);// Draw string
				break;
	...
	if(frameDraw ||(refresh && w->frameType==frame_outline)){
		switch(w->type){
			case widget_bmp_button:
				...
				ucg_DrawRectangle(&ucg, w->posX, w->posY, button->xbm[0]+4,  button->xbm[1]+4);
				break;
			case widget_editable:
			case widget_button:
			case widget_multi_option:
				...
				ucg_DrawRectangle(&ucg, w->posX ,w->posY, w->width, cHeight+1);
			default:
				break;
		}
...
uint8_t comboBoxDraw(widget_t *w) {
	...
	uint16_t yDim = ucg_GetYDim(&ucg) - w->posY;
	uint8_t height;
	int8_t frameY=0;
	int8_t posY;
	bool drawFrame=1;
	...
	height= ucg_GetFont(&ucg)->chars->image->height + 1;                 // +1 to allow separation between combobox items
	...
	for(uint8_t y = 0; y < yDim / height; ++y) {
		...
		if ((item->type==combo_Screen)||(item->type==combo_Action)){      // If screen or action item, just draw the label
			...
			if(item->dispAlign==align_left){
				ucg_WriteString(&ucg, 4, y * height + w->posY +2, item->text);
			}
			else{
				uint8_t len = ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), item->text);
				if(item->dispAlign==align_right){
					ucg_WriteString(&ucg, ucg_GetXDim(&ucg)-3-len, y * height + w->posY +2, item->text);
				}
				else{
					ucg_WriteString(&ucg, (ucg_GetXDim(&ucg)-1-len)/2, y * height + w->posY +2, item->text);
				}
			}
		}
			...
			if(sel->state==widget_edit){                                      // If edit mode
				drawFrame=0;
				ucg_SetForeColor(&ucg, w->fcolor);
				ucg_FillRectangle(&ucg, 0, frameY, ucg_GetXDim(&ucg), height);
				ucg_SetForeColor(&ucg, C_BLACK);
				ucg_SetBackColor(&ucg, w->fcolor);
			}
			else{
			...
			if(item->type==combo_MultiOption){
				len = ucg_GetStrWidth(&ucg, ucg_GetFont(&ucg), edit->options[*(uint8_t*)dis->getData()]);
			}
			else if(item->type==combo_Editable){
				if(dis->type==field_int32){
					int32_t val_ui = *(int32_t*)dis->getData();                         // Get data
			...
			dis->stringStart = ucg_GetXDim(&ucg)-len-5;                                   // Align to the left measuring actual string width
			if(item->type==combo_Editable){
				if((dis->type==field_string && (sel->state==widget_edit))){
					char str[sizeof(displayString)+1];
					uint8_t start,width;
				...
				ucg_WriteString(&ucg,dis->stringStart, posY+2,  edit->options[*(uint32_t*)dis->getData()]);
			}
			ucg_WriteString(&ucg, 4, y * height + w->posY +2, item->text);          // Draw the combo item label
		}
		do {
			item = item->next_item;
	...
	if(drawFrame){
		ucg_SetForeColor(&ucg, w->fcolor);
		ucg_DrawRectangle(&ucg, 0, frameY, ucg_GetXDim(&ucg), height);
	}
	return 1;
}
#endif
...
int comboBoxProcessInput(widget_t *w, RE_Rotation_t input, RE_State_t *state) {
	...
	uint16_t yDim = ucg_GetYDim(&ucg) - w->posY;
	uint16_t height = ucg_GetFont(&ucg)->chars->image->height + 1;
