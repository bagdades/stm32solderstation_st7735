...
void checksumError(uint8_t mode){
	Oled_error_init();
	ucg_WriteString(&ucg, 0, 0, "BAD CHECKSUM!");
	ucg_WriteString(&ucg, 0, 20, "RESTORING THE");
	if(mode==reset_Profile){
		ucg_WriteString(&ucg, 0, 36, "PROFILE");
	}
	else{
		ucg_WriteString(&ucg, 0, 36, "SYSTEM");
	}
...
void Button_reset(void){
	...
	if(!BUTTON_input()){
		Oled_error_init();
		ucg_WriteString(&ucg, 0, 10, "HOLD BUTTON");
		ucg_WriteString(&ucg, 0, 26, "TO RESTORE");
		ucg_WriteString(&ucg, 0, 42, "DEFAULTS");
		while(!BUTTON_input()){
			/* HAL_IWDG_Refresh(&hiwdg); */
			if((HAL_GetTick()-ResetTimer)>5000){
				...
				ucg_FillScreen(&ucg);
				ucg_SetForeColor(&ucg, C_CYAN);
				ucg_WriteString(&ucg, 0, 16, "RELEASE");
				ucg_WriteString(&ucg, 0, 32, "BUTTON NOW");
				while(!BUTTON_input()){
					/* HAL_IWDG_Refresh(&hiwdg); */
				}
				resetSystemSettings();
...
void ErrCountDown(uint8_t Start,uint8_t  xpos, uint8_t ypos){
	...
	while(Start){
		timErr=HAL_GetTick();
		ucg_SetForeColor(&ucg, C_BLACK);
		ucg_DrawRectangle(&ucg, xpos, ypos, ucg_GetStrWidth(&ucg, default_font, str), default_font->chars->image->height);
		ucg_SetForeColor(&ucg, C_CYAN);
		sprintf(&str[0],"%*u",length-1,Start);
		ucg_WriteString(&ucg, xpos, ypos, str);
